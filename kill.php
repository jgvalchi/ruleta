<?php
	if(isset($_POST["id"])){
		...
		$resultSelect = mysql_query("SELECT id, amount FROM premios WHERE id = ".$_POST["id"], $db);
		
		if($row = mysql_fetch_assoc($resultSelect)){
			$resultUpdate = mysql_query("UPDATE premios SET amount = ".($row["amount"]-1)." WHERE id = ".$_POST["id"], $db);
